...
def eval_wiff(folder_name, solver, vlsns_init_mode):
    result_file_path = os.path.join(os.getcwd(), folder_name, 'openms')
    ...
    coarse_registration_params = CoarseRegistrationParams(solver='ransac', mz_tolerance=0.01, use_ppm=False, centric_idx=0,
                                                          rt_tolerance=0.5, rt_residual_threshold=0.04, degree=1)
...
def eval_raw(folder_name, solver, vlsns_init_mode):
    ...
    coarse_registration_params = CoarseRegistrationParams(solver='ransac', mz_tolerance=0.005, use_ppm=False, centric_idx=0,
                                                          rt_tolerance=0.3, rt_residual_threshold=0.03, degree=1)
...
def eval_mtbls(folder_name, solver, vlsns_init_mode):
    result_file_path = os.path.join(os.getcwd(), folder_name, 'openms4')

    result_file_reading_params = ResultFileReadingParams(result_file_path, skip_line=0,
                                                         rt_col_num=2, mz_col_num=1, area_col_num=3)
    raw_file_reading_params = RawFileReadingParams()
    coarse_registration_params = CoarseRegistrationParams(solver='ransac', mz_tolerance=0.01, use_ppm=False, centric_idx=0,
                                                          rt_tolerance=0.3, rt_residual_threshold=0.05, degree=1)
    fine_assignment_params = FineAssignmentParams(rt_tolerance=0.1, mz_tolerance=0.01, mz_factor=1, rt_factor=1,
                                                  area_factor=1, use_ppm=False, solver=solver)
    if vlsns_init_mode is not None:
        fine_assignment_params.vlsns_solution_init_mode = vlsns_init_mode
...
# MTBLS562
# eval_mtbls('MTBLS562', solver='local_nearest', vlsns_init_mode=None)
eval_mtbls('MTBLS562', solver='local_bipartite', vlsns_init_mode=None)
# eval_mtbls('MTBLS562', solver='greedy', vlsns_init_mode=None)
# eval_mtbls('MTBLS562', solver='gurobi', vlsns_init_mode=None)
eval_mtbls('MTBLS562', solver='vlsns', vlsns_init_mode='msg')
eval_mtbls('MTBLS562', solver='vlsns', vlsns_init_mode='msr')