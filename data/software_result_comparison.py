...
wiff_lib_path = os.path.join(root_path, 'TripleTOF_6600', 'TripleTOF_6600_annotated.xlsx')
...
def load_third_party_aligned_result(result_path, area_col_idxes, separator=',', skip_line=1, rt_col_idx=1, mz_col_idx=0):
    result_file = open(result_path, 'r')
    for i in range(skip_line):
        header = result_file.readline().split(separator)
    result_data = np.array([line.strip().split(separator) for line in result_file])
    ...
    sorted_idx = np.argsort(mzs)
    aligned_matrix = np.concatenate([mzs[sorted_idx].reshape(-1, 1), rts[sorted_idx].reshape(-1, 1), areas[sorted_idx]], axis=1)
    return aligned_matrix
...
def match_aligned_area_to_result(aligned_matrix, result_matrix, area_idxes, mz_tolerance, use_ppm, rt_tolerance):
    match_matrix = np.ones((aligned_matrix.shape[0], len(area_idxes))).astype(np.int32) * -1
    for i in range(len(result_matrix)):
        sample_result = result_matrix[i]
        # aligned_result = aligned_matrix[:, [area_idxes[i] - 2, area_idxes[i] - 1, area_idxes[i]]]
        aligned_result = aligned_matrix[:, [0, 1, area_idxes[i]]]
...
def eval_galigner(lib_matrix, result_matrix, aligned_matrix, align_area_idxes, mz_tolerance=0.0001, rt_tolerance=0.0001):
    ...
    # ax[0].boxplot(mz_error_matrix.ravel())
    # ax[1].boxplot(rt_error_matrix.ravel())
    # plt.show()
    # print()
...
if __name__ == '__main__':
    ...
    def eval_mzmine2_wiff(wiff_lib_matrix):
        print('MZmine2 wiff')
        wiff_result_paths = [glob.glob(os.path.join(result_root_path, 'TripleTOF_6600', 'mzmine2',
                                                    '*' + name + '*'))[0] for name in wiff_sample_names]
        wiff_result_matrix = load_results(wiff_result_paths, separator=',', skip_line=1, rt_col_idx=1, mz_col_idx=0, area_col_idx=2)

        aligned_paths = [os.path.join(path, 'aligned_result.csv') for path in glob.glob(os.path.join(result_root_path, 'TripleTOF_6600_results_mzmine2', '[0-9]*'))]

        for aligned_result_path in aligned_paths:
            print(aligned_result_path)
            aligned_matrix = load_aligned_result(aligned_result_path)
            aligned_area_idxes = [6 + 3 * i for i in range(len(wiff_lib_matrix))]
            eval_galigner(wiff_lib_matrix, wiff_result_matrix, aligned_matrix, aligned_area_idxes, mz_tolerance=0.01, rt_tolerance=0.1)

        aligned_matrix = load_third_party_aligned_result(
            os.path.join(result_root_path, 'TripleTOF_6600_results_mzmine2', 'mzmine2_wiff_aligned_ransac.csv'),
            list(range(2, 2 + 3 * len(wiff_lib_matrix))))
        aligned_area_idxes = [4 + 3 * i for i in range(len(wiff_lib_matrix))]
        eval_galigner(wiff_lib_matrix, wiff_result_matrix, aligned_matrix, aligned_area_idxes, mz_tolerance=0.01, rt_tolerance=0.1)

    def eval_mzmine2_raw(raw_lib_matrix):
        print('MZmine2 raw')
        raw_result_paths = [glob.glob(os.path.join(result_root_path, 'QE_HF', 'mzmine2',
                                                   '*' + name + '*'))[0] for name in raw_sample_names]
        raw_result_matrix = load_results(raw_result_paths, separator=',', skip_line=1, rt_col_idx=1, mz_col_idx=0, area_col_idx=2)

        aligned_paths = [os.path.join(path, 'aligned_result.csv') for path in glob.glob(os.path.join(result_root_path, 'QE_HF_results_mzmine2', '[0-9]*'))]

        for aligned_result_path in aligned_paths:
            print(aligned_result_path)
            aligned_matrix = load_aligned_result(aligned_result_path)
            aligned_area_idxes = [6 + 3 * i for i in range(len(raw_lib_matrix))]
            eval_galigner(raw_lib_matrix, raw_result_matrix, aligned_matrix, aligned_area_idxes, mz_tolerance=0.005, rt_tolerance=0.1)

        aligned_matrix = load_third_party_aligned_result(
            os.path.join(result_root_path, 'QE_HF_results_mzmine2', 'mzmine2_raw_aligned_ransac.csv'),
    ...
    def eval_xcms_wiff(wiff_lib_matrix):
        print('XCMS wiff')
        wiff_result_paths = [glob.glob(os.path.join(result_root_path, 'TripleTOF_6600', 'xcms',
                                                    '*' + name + '*'))[0] for name in wiff_sample_names]
        wiff_result_matrix = load_results(wiff_result_paths, separator=',', skip_line=1, rt_col_idx=4, mz_col_idx=1, area_col_idx=7)
        aligned_paths = [os.path.join(path, 'aligned_result.csv') for path in glob.glob(os.path.join(result_root_path, 'TripleTOF_6600_results_xcms', '[0-9]*'))]
        ...
        aligned_matrix = load_third_party_aligned_result(
            os.path.join(result_root_path, 'TripleTOF_6600_results_xcms', 'xcms_aligned_group.csv'),
            list(range(10, 18)), rt_col_idx=3)
        aligned_area_idxes = list(range(2, 10))
        eval_galigner(wiff_lib_matrix, wiff_result_matrix, aligned_matrix, aligned_area_idxes, mz_tolerance=0.01, rt_tolerance=1)
        aligned_matrix = load_third_party_aligned_result(
            os.path.join(result_root_path, 'TripleTOF_6600_results_xcms', 'xcms_aligned_obiwarp.csv'),
            list(range(10, 18)), rt_col_idx=3)
        aligned_area_idxes = list(range(2, 10))
        eval_galigner(wiff_lib_matrix, wiff_result_matrix, aligned_matrix, aligned_area_idxes, mz_tolerance=0.01, rt_tolerance=1)

    def eval_xcms_raw(raw_lib_matrix):
        print('XCMS raw')
        raw_result_paths = [glob.glob(os.path.join(result_root_path, 'QE_HF', 'xcms',
                                                   '*' + name + '*'))[0] for name in raw_sample_names]
        raw_result_matrix = load_results(raw_result_paths, separator=',', skip_line=1, rt_col_idx=4, mz_col_idx=1, area_col_idx=7)

        aligned_paths = [os.path.join(path, 'aligned_result.csv') for path in glob.glob(os.path.join(result_root_path, 'QE_HF_results_xcms', '[0-9]*'))]
        ...
        aligned_matrix = load_third_party_aligned_result(
            os.path.join(result_root_path, 'QE_HF_results_xcms', 'xcms_aligned_group.csv'),
            list(range(10, 20)), rt_col_idx=3)
        ...
        aligned_matrix = load_third_party_aligned_result(
            os.path.join(result_root_path, 'QE_HF_results_xcms', 'xcms_aligned_obiwarp.csv'),
            list(range(10, 20)), rt_col_idx=3)
        aligned_area_idxes = list(range(2, 12))
        eval_galigner(raw_lib_matrix, raw_result_matrix, aligned_matrix, aligned_area_idxes, mz_tolerance=0.005, rt_tolerance=1)
    ...
    def eval_openms_wiff(wiff_lib_matrix):
        ...
        aligned_paths = [os.path.join(path, 'aligned_result.csv') for path in glob.glob(os.path.join(result_root_path, 'TripleTOF_6600_results_openms', '[0-9]*'))]
        ...
        aligned_matrix = load_third_party_aligned_result(
            os.path.join(result_root_path, 'TripleTOF_6600_results_openms', 'openms_aligned.csv'),
            [6 + 3 * i for i in range(len(wiff_lib_matrix))])
        aligned_area_idxes = list(range(2, 10))
        eval_galigner(wiff_lib_matrix, wiff_result_matrix, aligned_matrix, aligned_area_idxes, mz_tolerance=0.01, rt_tolerance=0.1)

    def eval_openms_raw(raw_lib_matrix):
        print('OpenMS raw')
        raw_result_paths = [glob.glob(os.path.join(result_root_path, 'QE_HF', 'openms',
                                                   '*' + name + '*'))[0] for name in raw_sample_names]
        raw_result_matrix = load_results(raw_result_paths, separator=',', skip_line=0, rt_col_idx=1, mz_col_idx=0, area_col_idx=2)

        aligned_paths = [os.path.join(path, 'aligned_result.csv') for path in glob.glob(os.path.join(result_root_path, 'QE_HF_results_openms', '[0-9]*'))]
        ...
        aligned_matrix = load_third_party_aligned_result(
            os.path.join(result_root_path, 'QE_HF_results_openms', 'openms_aligned.csv'),
            [6 + 3 * i for i in range(len(raw_lib_matrix))])
        aligned_area_idxes = list(range(2, 12))
        eval_galigner(raw_lib_matrix, raw_result_matrix, aligned_matrix, aligned_area_idxes, mz_tolerance=0.005, rt_tolerance=0.1)
