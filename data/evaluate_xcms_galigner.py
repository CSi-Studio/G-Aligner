...
def eval_wiff(folder_name, solver, vlsns_init_mode):
    result_file_path = os.path.join(os.getcwd(), folder_name, 'xcms')

    result_file_reading_params = ResultFileReadingParams(result_file_path, skip_line=1,
                                                         rt_col_num=5, mz_col_num=2, area_col_num=8)
...
def eval_mtbls(folder_name, solver, vlsns_init_mode):
    ...
    result_file_reading_params = ResultFileReadingParams(result_file_path, skip_line=1,
                                                         rt_col_num=4, mz_col_num=1, area_col_num=7)
    raw_file_reading_params = RawFileReadingParams()
    coarse_registration_params = CoarseRegistrationParams(solver='ransac', mz_tolerance=0.01, use_ppm=False, centric_idx=0,
