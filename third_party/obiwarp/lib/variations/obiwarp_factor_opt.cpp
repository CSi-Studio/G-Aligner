...
int mymain(int AXES, int BINARY, const char **logs, int nlogs, int IMAGES,
    ...
    strcat(resplotfilename, ".res.facopt_");
    ...
    fhres << "diagonal/gap ratio optimization (normalized smat)\n";
    fhres << "diagonal/gap score ratio (log_2)\n";
    fhres << "SUM(res^2)\n";

    char avgresplotfilename[1024];
    strcpy(avgresplotfilename, TESTFILE);
    strcat(avgresplotfilename, ".avgres.facopt_");
    ...
    fhavgres << "diagonal/gap ratio optimization (normalized smat)\n";
    fhavgres << "diagonal/gap score ratio (log_2)\n";
    fhavgres << "AVG(res^2)\n";

    char absplotfilename[1024];
    strcpy(absplotfilename, TESTFILE);
    strcat(absplotfilename, ".abs.facopt_");
    ...
    fhabs << "diagonal/gap ratio optimization (normalized smat)\n";
    fhabs << "diagonal/gap score ratio (log_2)\n";
    fhabs << "avg abs delta (sec)\n";

    // Make a zero gap penalty!
    VecF gp_array;
    dyn.linear_less_before(0,0,gp_length,gp_array);
    //gp_array.print();
    ...
    for (int x = 0; x < xaxis.length(); ++x) {
        float diag_gap_ratio = pow(2, (xaxis[x]));
        float diag_factor = diag_gap_ratio;
        float gap_factor = 1.f;
