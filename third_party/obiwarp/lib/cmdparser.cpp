    ...
    for (_i = 1; _i < argc; ++_i) {
        ...
        if (argv[_i][0] == '-') {
            if (argv[_i][1] == '-') {
                larg = &(argv[_i][2]);
                //printf("LONGARG: %s\n", larg);
                if (eq(larg,(char *)"format")) {set_string(&format);}
                else if (eq(larg,(char *)"outfile")) {set_string(&outfile);}
                else if (eq(larg,(char *)"images")) {set_flag(images);}
                else if (eq(larg,(char *)"timefile")) {set_string(&timefile);}
                else if (eq(larg,(char *)"score")) {set_string(&score);set_defaults_by_score(score);}
                else if (eq(larg,(char *)"local")) {set_flag(local);}
                else if (eq(larg,(char *)"nostdnrm")) {set_flag(nostdnrm);}
                else if (eq(larg,(char *)"factor")) {set_comma_list_float(factor_diag, factor_gap);}
                else if (eq(larg,(char *)"gap")) {set_comma_list_float(gap_init, gap_extend);}
                else if (eq(larg,(char *)"init")) {set_float(init_penalty);}
                else if (eq(larg,(char *)"response")) {set_float(response);}
                else if (eq(larg,(char *)"help")) {help_func(argc); exit(1);}
                else if (eq(larg,(char *)"version")) {print_version(version); exit(1);}
                else if (eq(larg,(char *)"smat_in")) {set_string(&smat_in);}
                else if (eq(larg,(char *)"smat_out")) {set_string(&smat_out);}
                else if (eq(larg, (char *)"diagnostics")) {diagnostics = 1;}
                else {
                    std::cerr << "********************************************\n";
